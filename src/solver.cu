...
namespace pl {
	...
	template <>
	[[nodiscard]] Multiexp solveBatch<transformType::Trapezia> (
		const dataset_type& h_data, unsigned depth)
	{
      ...
      return a.point < b.point;
		...
		kernelBatchVecSolver <<< d_grid.size(), depth, 4 * depth * sizeof(complex) >>> (
      d_grid.data().get(),
      d_result.data().get(),
